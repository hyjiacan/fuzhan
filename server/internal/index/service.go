...
func (s *Service) StartScan() error {
    return s.StartScanByScope(ScanScopePublic)
}

// StartScanByScope 启动指定 scope 的全量扫描
// 扫描完成后自动触发哈希计算
// 注意：临时文件不建立索引，直接跳过（在 Scanner.StartScanScope 中处理）
func (s *Service) StartScanByScope(scope ScanScope) error {
    // 使用独立的 Background Context，不绑定 HTTP 请求生命周期
    ctx, cancel := context.WithTimeout(context.Background(), 24*time.Hour)
    err := s.scanner.StartScanScope(ctx, scope)
    if err != nil {
        cancel()
        return err
    }
