...
// TestKeepDuplicate_UpdatesListTotal 验证执行"保留"删除其他重复文件后，
// 再次查询重复分组时 total 会同步减少（与 groups 一致，反映处理后的状态）。
func TestKeepDuplicate_UpdatesListTotal(t *testing.T) {
	root := t.TempDir()
	db := setupTestDB(t)
	svc := NewService(db, map[string]string{"docs": root}, "", "")

	id1 := createTestRecord(db, "a.txt", "/a.txt", "docs", "samehash", 100)
	createTestRecord(db, "b.txt", "/b.txt", "docs", "samehash", 100)
	...
	if total != 1 || len(groups) != 1 {
		t.Fatalf("期望处理前 1 组重复，实际 total=%d groups=%d", total, len(groups))
	}

	// 保留 a.txt，删除同哈希的 b.txt
	res, err := svc.KeepDuplicate(id1)
	if err != nil {
		t.Fatalf("保留失败: %v", err)
	}
	if len(res.DeletedFiles) != 1 {
		t.Fatalf("期望删除 1 个重复文件，实际 %d", len(res.DeletedFiles))
	}

	groups, total, err = svc.ListDuplicateGroups(DuplicateQuery{})
	if err != nil {
		t.Fatalf("处理后再查询失败: %v", err)
	}
	if total != 0 {
		t.Errorf("处理后 total 应降为 0，实际 %d", total)
	}
	if len(groups) != 0 {
		t.Errorf("处理后 groups 应为空，实际 %d 组", len(groups))
	}
}
