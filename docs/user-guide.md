...
界面主题跟随系统自动适配，同时支持手动切换：

- 公共端：点击顶栏右上角的主题图标，可在「跟随系统 / 亮色 / 暗色」间切换。
- 管理端：点击内容区右上角悬浮的圆形主题按钮，同样支持三种模式切换。
- 选择会被记住并持久化；不同标签页之间自动同步。

主题偏好保存在浏览器本地（`localStorage`），仅影响当前浏览器，不会更改服务器端配置。
